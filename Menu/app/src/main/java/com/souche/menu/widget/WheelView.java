...
package com.souche.menu.widget;
...
import com.souche.menu.Utils.DensityUtils;
import com.souche.menu.widget.adapters.WheelViewAdapter;
...
public class WheelView extends View {
    ...
    private static final int[] SHADOWS_COLORS = new int[]{0xFFFFFFFF,
            0xDDFFFFFF, 0x80FFFFFF};

    /**
     * 中间线条颜色
     */
    private static final int CENTER_LINE_COLOR = 0x80C9CACA;
    /**
     ...
     */
    private final int PADDING = DensityUtils.dip2px(getContext(), 4);
    private static final int LABEL_TEXT_SIZE = 16;
    /**
     * Default count of visible items
     */
    private static final int DEF_VISIBLE_ITEMS = 5;
    private float rightLabelLen = 0;
    private float leftLabelLen = 0;
    // Wheel Values
    ...
    private void initData(Context context) {
        labelPaint.setTextSize(DensityUtils.dip2px(getContext(),
                LABEL_TEXT_SIZE));
    ...
    public void setLabel(String left, String right) {
        mLeftLabel = left;
        mRightLabel = right;
        if (StringUtils.isBlank(mLeftLabel)) {
            leftLabelLen = 0;
        } else {
            leftLabelLen = labelPaint.measureText(mLeftLabel);
        }
        if (StringUtils.isBlank(mRightLabel)) {
            rightLabelLen = 0;
        } else {
            rightLabelLen = labelPaint.measureText(mRightLabel);
        }
        invalidate();
    }
    ...
    private void drawItems(Canvas canvas) {
        ...
        if (zoomCenter) {
            ...
            canvas.clipRect(0, 0, getWidth(), center - offset,
                    Op.REPLACE);
            ...
            canvas.save();
            canvas.translate(0, 0);
            canvas.clipRect(0, center - offset + 1, getWidth(), center + offset,
                    Op.REPLACE);
            canvas.scale(1.1F, 1.1F, getWidth() / 2, center);
            canvas.translate(PADDING, -top + scrollingOffset);
            itemsLayout.draw(canvas);
            canvas.restore();
            //下
            canvas.save();
            canvas.translate(0, 0);
            canvas.clipRect(0, center + offset, getWidth(), getHeight(),
                    Op.REPLACE);
            canvas.translate(PADDING, -top + scrollingOffset);
            itemsLayout.draw(canvas);
            canvas.restore();
    ...
    private void drawCenterRect(Canvas canvas) {
        int center = getHeight() / 2;
        int offset = (int) (getItemHeight() / 2 * 1.2);
        Paint rectPaint = new Paint(Paint.DITHER_FLAG);
        rectPaint.setStrokeWidth(1);// 笔宽1像素
        rectPaint.setColor(CENTER_LINE_COLOR);
        rectPaint.setAntiAlias(true);// 锯齿不显示
    ...
    public void setZoomCenter(boolean zoom) {
        this.zoomCenter = zoom;
        invalidate();
    }
}
